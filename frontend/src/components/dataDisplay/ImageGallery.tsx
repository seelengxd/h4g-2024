import { ArrowLeftCircleIcon, ArrowRightCircleIcon, XCircleIcon } from "@heroicons/react/20/solid";
...
const ImageGallery: React.FC<ImageGalleryProps> = ({ imageUrls, height = "h-56", deletable = false, onDelete }: ImageGalleryProps) => {
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className={`relative w-full ${height} overflow-hidden rounded-lg`}>
        {imageUrls.map((imageUrl, imgIndex) => (
            <div className={imgIndex !== index ? " hidden" : ""}>
              ...
              {deletable && (
                <XCircleIcon className="absolute top-2 right-2 w-10 h-10 fill-gray-700 hover:fill-red-700" onClick={() => {
      ...
      {isDialogOpen && (
        <ConfirmationDialog
          ...
          onDelete={() => {
            const updatedImageUrls = imageUrls.filter((_item, index) => index !== deleteIndex);
