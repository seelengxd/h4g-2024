...
const Tabs: React.FC<TabsProps> = ({
  ...
  const activeTabClass =
    "inline-block p-4 text-primary-600 bg-primary-200 rounded-t-lg active";
  const inactiveTabClass =
    "inline-block p-4 rounded-t-lg bg-gray-50 hover:text-gray-600 hover:bg-gray-200";
        ...
        <ul className="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b-2 border-primary-300">
          {tabs.map((tab) => (
            <li className="cursor-pointer me-2">
              <span
                className={
                  tab.id === activeTabId ? activeTabClass : inactiveTabClass
                }
