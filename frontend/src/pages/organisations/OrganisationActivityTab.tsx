...
const OrganisationActivityTab: React.FC<OrganisationActivityTabProps> = ({ activities }: OrganisationActivityTabProps) => {
  ...
  <div className="mt-6 mb-12">
    <div className="flex justify-between items-center">
      <h4 className="font-semibold text-xl">{activities.length} Activities</h4>
    ...
    <div className="mt-4" hidden={isTableView}>
      <div className="grid grid-cols-4 gap-x-8 gap-y-16">
          {activities.map((activity) => <VolunteeringOpportunityCard activity={activity} />)}
      </div>
    </div>
  </div>
 );
