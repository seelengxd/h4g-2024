...
const ImmigrationStatusTab: React.FC<Props> = ({ report }) => {
  ...
  const pieChartOptions = {
    ...
    theme: "light2",
    title: {
      text: "Volunteers By Immigration Status",
    },
    legend: {
      cursor: "pointer",
    },
    data: [
      {
        ...
        indexLabel: "{label} {y}",
        dataPoints: report.immigrationStatus.dataPoints.map((dataPoint) => ({
          ...dataPoint,
  ...
  const immigrationStatus = report.immigrationStatus.dataPoints.map(
    (row, index) => ({
      label: row.label,
      value: index,
    })
  ...
  return (
    <>
      ...
      <DataTable
        ...
        tableData={report.immigrationStatus.users[categoryIndex].users}
...
export default ImmigrationStatusTab;
